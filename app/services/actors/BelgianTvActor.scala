...
class BelgianTvActor(
  ...
  private val tomatoesThrottler = {
    ...
    Source
      .actorRef(bufferSize = 1000, OverflowStrategy.fail)
      .throttle(2, 1.second)
  ...
  def receive: Receive = {
    ...
    case msg: LinkImdb =>
      logger.info(s"[$this] - Received [$msg] from ${sender()}")
      val res = movieRepository
        .find(msg.broadcast.name, msg.broadcast.year)
        .flatMap {
          ...
          msg.events.foreach { broadcast =>
            if (broadcast.yeloUrl.isEmpty) {
              ...
              found
                ...
                .getOrElse {
                  logger
                    .warn("No yelo match for " + broadcast.channel + " " + broadcast.humanDate + " " + broadcast.name)
                }
            }
          }
      }
          ...
          msg.events.foreach { broadcast =>
            if (broadcast.belgacomUrl.isEmpty) {
              ...
              found
                ...
                .getOrElse {
                  logger.warn(
                    "No belgacom match for " + broadcast.channel + " " + broadcast.humanDate + " " + broadcast.name
                  )
                }
            }
          }
