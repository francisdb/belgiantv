...
class MovieRepository(reactiveMongoApi: ReactiveMongoApi, executionContext: ExecutionContext) extends MongoSupport {
  ...
  def find(name: String, year: Option[Int] = None): Future[Option[Movie]] = movieCollectionFuture.flatMap {
    movieCollection =>
      year match {
  ...
  def findByNameAndYear(name: String, year: Int): Future[Option[Movie]] = movieCollectionFuture.flatMap {
    movieCollection =>
      movieCollection
        .find(
          Json.obj(
